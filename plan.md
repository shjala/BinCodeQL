## Plan: LLM+MCP+Datalog Binary Query Agent (v2)

SUPERSEDED: The previous v1 plan has been revised into bin_datalog/plan-v2.md based on critical review. Key changes: added MLIL-SSA-centric extraction, added Phase 0 spike, cut v1 scope (BN-only, no provenance, 3 rule modules), identified MCP text parsing as the core engineering challenge, added competitor framing (Joern, CodeQL), grounded in 5 concrete user queries.
